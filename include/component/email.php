...
function _sendMail($from, $to, $subject, $body, $ccmail = "", $bccmail = "") {
    
    global $MAILER;
    

    if(!isset($MAILER[$from])){
        echo $from." not in email list";
    }
    ...
    $mail->SMTPDebug = 0;
    $mail->CharSet = 'utf-8'; 
    $mail->Host = $from["REPLY_EMAIL_HOST"];
    $mail->SMTPAuth = true;
    $mail->Port = 465;
    $mail->SMTPSecure = "ssl";
    $mail->Username = $from["REPLY_EMAIL_USERNAME"];
    $mail->Password = $from["REPLY_EMAIL_PASSWORD"];
    $mail->From = $from["REPLY_EMAIL_FROM"];
    $mail->FromName = $from["REPLY_EMAIL_FROMNAME"];
    if(is_array($to)){
        foreach($to as $t){
            $mail->addAddress($t);
        }
...
function sendMail($from,$to,$subject,$body,$cc = null){
    global $MAILER;
    startMQ();
    if(!isset($MAILER[$from])){
        echo("Error: \$MAILER has no email $from");
        return;
    }

    $info = ["From"=>$from,"To"=>$to,"Subject"=>$subject,"Body"=>$body,"CC"=>$cc];
    redisLPush("SEND_MAIL", json_encode($info));
